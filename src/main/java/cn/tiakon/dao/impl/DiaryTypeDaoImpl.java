...
public class DiaryTypeDaoImpl implements DiaryTypeDao {
    ...
    public List<DiaryType> diaryTypeCountList(Connection connection) throws SQLException {
        String sql = "SELECT t_diarytype.`diary_typeid`,  t_diarytype.`typename`,  COUNT(t_diary.`title`) AS `count` FROM  t_diary  RIGHT JOIN t_diarytype ON t_diary.`typeid` = t_diarytype.`diary_typeid` GROUP BY t_diarytype.`diary_typeid`,t_diarytype.`typename` ORDER BY diary_typeid DESC";
